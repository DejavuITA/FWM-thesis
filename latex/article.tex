...
One of the researchers, who would later become my advisor, suggested me a few interesting topics in guided-wave optics which could be studied as a undergraduate thesis.
...
The second equation (\ref{eq_wg_b}), on the other hand, defines the change of field distribution along the $\hat{z}$ axis, or the waveguide axis.
...
Our study focuses on a special condition of the second case, where $\omega_1 = \omega_2 = \omega_P$. The setup therefore consists of a strong pump of frequency  $\omega_P$ and a signal of frequency  $\omega_3 = \omega_S$, resulting in the creation of the idler wave of frequency $\omega_4 = \omega_I$.
...
In order to be efficient, the parametric process needs to verify the conservation of both energy and momentum, as given in Eq. (\ref{eq_f&pmc}).
...
Because the degrees of freedom of the problem were too many, we resorted to automated algorithms which gave us a numerical, and therefore approximate, solution.
...
\begin{figure}[h!]
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{Peaks position at $T_{amb}$.}
...
It should not be easily considered as negligible contribution as it could change the waveguide geometry and therefore the modes.
