...
\begin{equation}
	\textbf{P} = \varepsilon_0 \left( \chi^{(1)} \cdot \textbf{E} + \chi^{(2)} : \textbf{E}^2 + \chi^{(3)} \vdots \textbf{E}^3 + \cdots \right) = \textbf{P}_L + \textbf{P}_{NL}
	\label{eq_P_general}
\end{equation}
...
We can then write the non linear terms $P_{NL}$ of the induced electric polarization as
\begin{equation}
	\textbf{P} = \frac{1}{2}\hat{x} \sum_{j=1}^4 P_j \mathrm{exp}[i(\beta_j z - \omega_j t)] + c.c.
	\label{eq_P}
\end{equation}

Replacing \ref{} in Eq. (\ref{eq_helm_P}) leads to eight coupled equation: four of them are equivalent of Eq. (\ref{eq_wg_a}) and describe the field distribution of the mode of each wave.
The latter four are the equivalent of Eq. (\ref{eq_wg_b}) and describe how the field change while travelling through the waveguide.
...
\end{equation*}
with $\kappa = \Delta + \gamma(P_1 + P_2)$ \textit{effective phase mismatch} and $g = \sqrt{4\gamma^2P_1P_2 - \kappa^2/4}$ the \textit{parametric gain}.
