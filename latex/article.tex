...
\begin{align}
	...
	\frac{\partial^2A(z,t)}{\partial z^2} + \frac{n^2}{c^2}\frac{\partial^2A(z,t)}{\partial t^2} = 0
	\quad ??? \quad or \quad ??? \quad
	\frac{\partial^2A(z,t)}{\partial z^2} + k^2n^2\frac{\partial^2A(z,t)}{\partial t^2} = 0
\end{align}
\end{subequations}
where $k_0$ is the wavevector in vacuum and $n$ is a function of the position (mainly $x$ and $y$).

\vspace{18pt}
************************************************************************
\vspace{12pt}

These equations, once solved, lead to the distribution of the fields in the plane xy and to their modification along the z axis.
...
Using the last two equations and the undepleted pump approximation, (\ref{eq_coupled_amplitude}a) and (\ref{eq_coupled_amplitude}b) are easily solved to obtain the pump fields
\begin{subequations}
\begin{align}
...
Using the same approximations on (\ref{eq_coupled_amplitude}c) and (\ref{eq_coupled_amplitude}d) and replacing eq (\ref{eq_pump_sol}) where needed we obtain two linear coupled equation for signal and idler:
\begin{subequations}
\begin{align}
...
The \textit{effective} phase mismatch is therefore given by
\begin{equation}
	\kappa = \Delta k + \gamma(P_1 + P_2)
\end{equation}
