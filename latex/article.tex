...
\begin{figure}[ht]
	\centering
	\includegraphics[width=.55\textwidth]{geometry.png}
...
We positioned a thermal reservoir, a ``\textit{heater}", \SI{0.3}{\um} wide at the top of the cladding, exactly over the center of the waveguide core and set to mantain a specific temperature in the gap, given by the configuration chosen.
To 
  ...
  \begin{tabular}{lcccc}  
    ...
    \cmidrule(r){2-5}
    	\multirow{2}{*}{Materials}    & $\rho$ & $\kappa_{TH}$ & $C_P$ & TOC\\
  		& [\si{\kg\m^3}] & [\si{\W\per\m\per\K}] & [\si{\J\per\kg\per\K}] & [\si{\per\K}] \\
    \midrule
...
\end{equation}
where $\mathrm{TOC}|_{\mathrm{Si}} = \SI{1.87e-4}{\per\K}$\cite{n_db} is called Termo-Optic Coefficient. The coefficient of silica glass is negligible and coefficient was therefore considered zero $\mathrm{TOC}|_{\mathrm{SiO}_2} \approx 0$.
...
We emploied an automated algorithm to solve, in the frequency domain, the Helmholtz equation on a fine grid in the waveguide section.
...
$$n_{eff} = f_{mt,\,mo} \left( \omega, T \right) = f'_{mt,\,mo} \left( \lambda, T \right)$$
...
We replaced its value in the conservation of momentum formula, giving us the phase mismatch as a function only of signal wavelength and temperature of the heater, for each set of orders.
...
After the selection of sets, we evaluated the dependence of each maximum on temperature.
