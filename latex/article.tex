...
I chose to study the the temperature behaviour of silicon waveguides.
...
Therefore it is about two waves (pump and signal) which generate a third wave (idler).
...
Then we fit effective index data for each parametric configuration with a polinomial function of frequency $\omega = 2\pi \nu$ and temperature $T$:
$$n_{eff} = f_{mt,\,mo} \left( \omega, T \right)$$
where $mt$ and $mo$ means respectively \textit{mode type} (TE or TM) and \textit{mode order} (0, 1, 2, ...)
