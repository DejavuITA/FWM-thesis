...
Figure (\ref{fig_ex_mode}) portrays a TE configuration of the third order.

\begin{figure}[hb]
	\centering
	\includegraphics[width=0.85\textwidth]{output_mode.pdf}
	\caption{Example of third order TE mode in a \SI{3}{\um} wide waveguide}
	\label{fig_ex_mode}
\end{figure}
...
To make the graphs easier to read, we excluded set of data the points of no interest: those which peak had the same frequency $\omega_P = \SI{1.55}{\um}$ of the pump.

The first Figure (\ref{fig_sym_pp@t2}) tell us where we found phase-matching condition.
We can see that we have only two condition in which the phasematch is verified.
Also, combination \texttt{517} is the symmetric of \texttt{571}.

\begin{figure}[!h]
	\centering
	\includegraphics[width=.85\textwidth]{ppaat2.png}
	\caption{Peak position at $T_{amb}$. The label of the x axis are to be read as pump-signal-idler orders. We have two TE conbinations and two TM combinations.}
	\label{fig_sym_pp@t2} 	% pp@t = bandwidth @ T_amb, 2 = no points at 1.55um
\end{figure}

The second Figure (\ref{fig_sym_ppc2}) shows us that thant the change in position is of a fraction of \si{\nm}.
...
\begin{figure}[!h]
	...
	\caption{Change of peak position. The label of the x axis are to be read as pump-signal-idler orders. We have two TE conbinations and two TM combinations.}
...
\begin{figure}[!h]
	...
	\caption{Bandwidth at $T_{amb}$. The label of the x axis are to be read as pump-signal-idler orders. We have two TE conbinations and two TM combinations.}
...
\subsubsection{data aggregation}
Unfortunately, examining the graphs obtained from the data analysis of the asymmetric configuration, we do not noticed anything new.
...
\subsection{Summary}
We had little results in the frequency gap chosen.
This because we probably need to study further the geometry of the waveguide, in order to enhance the effect of the overlap integrals between the optical mode and the thermal profile.
